...
double quaternion_to_yaw( const geometry_msgs::Quaternion& quaternion ){
	return atan2( 2.0 * ( quaternion.w * quaternion.z + quaternion.x *
	quaternion.y ), 1.0 - 2.0 * ( quaternion.y * quaternion.y + quaternion.z* quaternion.z ) );
}
...
OCMap::OCMap( const double& discretizationArg,const unsigned int& widthArg,const unsigned int& heightArg ) : ocmap(),discretization( discretizationArg ),xs( widthArg ),ys( heightArg ),l0( log( 0.5 / ( 1.0 - 0.5 ) ) ),locc( log( 0.95 / ( 1.0 - 0.95 ) ) ), lfree( log( 0.05 / ( 1.0 - 0.05 ) )) {
...
OCMap::~OCMap() {}
void OCMap::handle_laser(const sensor_msgs::LaserScan::ConstPtr &msg){
	_laser = *msg;
	return;
}

void OCMap::handle_odom(const nav_msgs::Odometry::ConstPtr &msg){
	_odom = *msg;
	return;
}

bool OCMap::checkMap( const double& x,const double& y,const double& radius,const double& threshold ){
...
void OCMap::update( const geometry_msgs::Pose& pose, const sensor_msgs::LaserScan& scan ){
...
// updating occupied _map log probability, left as an exercise for you,revisit the probabilistic robotics chapter on occupancy grid mapping fordetails about how to implement this
	for(int x = 0; x < free_cells.size(); x++){
		ocmap.data[free_cells[x]] = double_to_int8(int8_to_double(ocmap.data[free_cells[x]]) + lfree -l0);
	}
	for(int x = 0; x < occupied_cells.size(); x++){
                ocmap.data[occupied_cells[x]] = double_to_int8(int8_to_double(ocmap.data[occupied_cells[x]]) + locc -l0);
        }

	return;
}

ostream& operator<<( ostream& out,const OCMap& other ){
return out;
}
